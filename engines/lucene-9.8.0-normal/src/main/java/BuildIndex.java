...
public class BuildIndex {
	...
	public static void main(String[] args) throws Exception {
		...
		if (Files.exists(outputPath)) {
			System.out.println("Index [" + outputPath + "] already exists");
			return;
		}

		final boolean useDefaultOptions = args.length == 1 || args[1].equals("default");
		System.out.println("Build DOCS_AND_FREQS_AND_POSITIONS? - " + useDefaultOptions);
		...
		try (Directory dir = FSDirectory.open(outputPath);
			...
			for (int i = 0; i < threads.length; ++i) {
				...
				if (useDefaultOptions) {
					textField = new TextField("text", "", Field.Store.NO);
				} else {
					FieldType fieldType = new FieldType();
					fieldType.setIndexOptions(IndexOptions.DOCS_AND_FREQS);
					fieldType.setTokenized(true);
					fieldType.freeze();
					textField = new Field("text", "", fieldType);
				}
